...
unittestPam8TSignatureInterpreter = do
  ...
  let state1 = execute testfunmod (SetVar "x" (Lit 11 "NOK")) newState
  let state2 = execute testfunmod (SetVar "y" (Lit 10 "gram")) state1
  let state3 = execute testfunmod (SetVar "z" (Lit 23 "meter")) state2
  -- Correct the values for the variables out of order.
  let state4 = execute testfunmod (AssVar "y" (Lit 37 "gram/meter")) state3
  let state5 = execute testfunmod (AssVar "z" (Lit 39 "gram")) state4
  let state6 = execute testfunmod (AssVar "x" (Lit 31 "NOK/Length")) state5
