...
calculatorTemplate intrinsics@(types,fundecls) funmod testfun valtyp = do
  ...
  loop tenv state = do
    ...
    case input of
      Nothing -> return ()
      Just "H" -> 
        do outputStrLn $ "Here you go" ++ (show fundecls); 
      Just "" ->
      ...
      Just str -> do
        case readMaybe str of
          Nothing -> do
            ...
            outputStrLn $ "Press H for all functions"
            loop tenv state

          -- is declared variable name from the set var statement 
          Just stmt@(SetVar vname expr)|isDeclared vname state -> do
            outputStrLn $ "Error: output variable " ++ (show vname) ++ " already exists."
            outputStrLn $ "Your variable is assigned to: " ++ (show vname) ++ " = "++ (show (getValue vname state))
